...
def getBaseModel(input_shape, units, layers_number, hidden_layers_activation="relu"):
    ...
    dense = layers.Dense(units=units, activation=hidden_layers_activation,)(input_layer)

    for i in range(layers_number - 1):
        dense = layers.Dense(units=units - 3*i, activation=hidden_layers_activation,)(dense)
