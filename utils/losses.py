...
def weighted_binary_crossentropy_per_output(weights):
    """
    Returns a weighted binary crossentropy loss function with weights per output dimension.
    
    Args:
    - weights: A tensor or list of weights corresponding to each output dimension.
    """
    ...
    def loss(y_true, y_pred):        
        ...
        # Compute the binary cross-entropy for each output
        loss = - (y_true * tf.math.log(y_pred) + (1 - y_true) * tf.math.log(1 - y_pred))


        # Apply the weights to each output dimension
